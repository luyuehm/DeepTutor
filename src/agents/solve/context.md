...
| **user → {preference}** | `PersonalizationService.get_preference_for_prompt()` | 用户学习偏好（可为空） |
...
| 注入点 | 内容 | 条件 |
|--------|------|------|
| WriterAgent.process() → `{preference}` | `LearningMemory.get_preference_context()` | 仅 Simple 模式；服务可用时 |
| PlannerAgent — **未注入** | — | — |
| SolverAgent — **未注入** | — | — |
