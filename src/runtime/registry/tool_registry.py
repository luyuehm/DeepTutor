...
from src.core.tool_protocol import BaseTool, ToolDefinition, ToolPromptHints
from src.runtime.bootstrap.builtin_tools import BUILTIN_TOOL_CLASSES, TOOL_ALIASES
from src.tools.prompting.composer import ToolPromptComposer

logger = logging.getLogger(__name__)


def _import_tool_class(path: str) -> type[BaseTool]:
    """Import a tool class from a 'module.path:ClassName' string."""
...
class ToolRegistry:
    ...
    def load_builtins(self) -> None:
        ...
        for name, class_path in BUILTIN_TOOL_CLASSES.items():
            if name in self._tools:
                continue
            try:
                cls = _import_tool_class(class_path)
                self.register(cls())
            except Exception:
                logger.warning("Failed to load built-in tool %s", name, exc_info=True)
