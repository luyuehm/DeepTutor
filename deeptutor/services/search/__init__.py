...
from deeptutor.services.config import PROJECT_ROOT, get_env_store, load_config_with_main
...
def web_search(
    ...
    **provider_kwargs: Any,
    ...
    # Determine provider: function arg > .env > default
    provider_name = (provider or get_env_store().get("SEARCH_PROVIDER") or "perplexity").lower()
...
def get_current_config() -> dict[str, Any]:
    ...
    provider = (
        get_env_store().get("SEARCH_PROVIDER") or config.get("provider") or "perplexity"
    ).lower()
    ...
    return {
        ...
        "config_source": "env"
        if get_env_store().get("SEARCH_PROVIDER")
        else "yaml"
