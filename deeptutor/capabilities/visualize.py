    ...
    async def _run_answer_now(
        ...
        if is_zh:
            system_prompt = (
                ...
                "\"code\": \"...\"}。"
                "code 字段是可直接渲染的源代码："
                "SVG 标签 / Chart.js JS 配置 / Mermaid 文本 / "
                "完整的单文件 HTML（从 <!DOCTYPE> 到 </html>，"
                "CSS 内联在 <style>，JS 内联在 <script>，不依赖外部资源，"
                "页面会渲染在 iframe 中，KaTeX 由宿主注入）。"
            )
            user_prompt = (
                f"用户请求：{original}\n\n"
            ...
            system_prompt = (
                ...
                "\"svg|chartjs|mermaid|html\", \"code\": \"...\"}, where ``code`` "
                "is the renderable source: SVG markup / Chart.js JS config / "
                "Mermaid DSL / a complete single-file HTML document "
                "(from <!DOCTYPE> to </html>, CSS inlined in <style>, JS in "
                "<script>, no external resources; the page renders inside an "
                "iframe and KaTeX is injected by the host)."
            )
            user_prompt = (
